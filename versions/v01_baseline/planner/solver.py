...
def _heuristic_with_distances(
    ...
    """Hungarian-style matching heuristic: minimum-cost assignment of boxes to
    goals, using reverse-push BFS distances.  Admissible and tighter than
    the simple sum heuristic because it avoids double-counting the same goal.
    """
    n = len(boxes_config)
    if n == 0:
        return 0
    if n == 1:
        box_id, box_pos = boxes_config[0]
        goal_pos = board.goals[box_id]
        gd = wall_dist.get(box_id, {})
        return gd.get(box_pos, abs(box_pos[0] - goal_pos[0]) + abs(box_pos[1] - goal_pos[1]))

    # Build cost matrix: cost[i][j] = distance to push box i to goal j
    goal_ids = sorted(board.goals)
    cost_matrix: list[list[int]] = []
    for box_id, box_pos in boxes_config:
        row_costs: list[int] = []
        for gid in goal_ids:
            goal_pos = board.goals[gid]
            gd = wall_dist.get(gid, {})
            if box_pos in gd:
                c = gd[box_pos]
            else:
                c = abs(box_pos[0] - goal_pos[0]) + abs(box_pos[1] - goal_pos[1])
            row_costs.append(c)
        cost_matrix.append(row_costs)

    # For n <= 4, brute-force all permutations is fast enough (4! = 24)
    if n <= 4:
        return _min_assignment_cost(cost_matrix)

    # Fallback: greedy matching (still admissible as a lower bound via min per box)
    total = 0
    used_goals: set[int] = set()
    for i in range(n):
        best = min(
            (cost_matrix[i][j] for j in range(len(goal_ids)) if j not in used_goals),
            default=0,
        )
        total += best
        # Mark the cheapest goal as used (greedy)
        best_j = min(
            (j for j in range(len(goal_ids)) if j not in used_goals),
...
def _min_assignment_cost(cost_matrix: list[list[int]]) -> int:
    """Brute-force minimum assignment for small n (<=4)."""
    n = len(cost_matrix)
    if n == 0:
        return 0
    m = len(cost_matrix[0])
    ...
    def _permute(path: list[int], remaining: list[int]) -> None:
        nonlocal best
        if len(path) == n:
            total = sum(cost_matrix[i][path[i]] for i in range(n))
            if total < best:
                best = total
            return
        for j in remaining:
            path.append(j)
            _permute(path, [r for r in remaining if r != j])
            path.pop()
...
def _bomb_heuristic(
    ...
    BFS distances would be inadmissible.  Instead, we combine:
    1. Manhattan distance to goals (admissible lower bound).
    2. A bonus for bombs adjacent to walls that block box paths, weighted at
       half the Manhattan distance the wall would save (rounded down → admissible).
    3. Penalty for bombs blocking box corridors (encourages moving them early).
    """
    if not bombs_tuple:
        # No bombs → walls are final, use wall-aware BFS
    ...
    for box_id, box_pos in boxes_config:
        goal_pos = board.goals[box_id]
        md = abs(box_pos[0] - goal_pos[0]) + abs(box_pos[1] - goal_pos[1])
        estimate += md

    # Bonus: if a bomb is adjacent to a wall, and that wall lies on the
    # Manhattan rectangle between a box and its goal, the explosion could
    # shortcut.  Give a half-credit (floor) per such wall cell.
    wall_set = set(walls)
    bomb_bonus = 0
    for bomb_pos in bombs_tuple:
        ...
        for r_delta, c_delta, _direction in DIRECTIONS:
            ...
            if wall_cell not in wall_set:
                continue
            # Check if this wall lies on any box-goal Manhattan corridor
            for box_id, box_pos in boxes_config:
                goal_pos = board.goals[box_id]
                if _on_manhattan_path(box_pos, goal_pos, wall_cell):
                    # Half the wall distance saved (rounded down) — admissible
                    bomb_bonus = max(bomb_bonus, 1)
    estimate = max(0, estimate - bomb_bonus)

    # Penalty: bombs sitting on box-goal paths that are directly blocking
    for bomb_pos in bombs_tuple:
        for box_id, box_pos in boxes_config:
            goal_pos = board.goals[box_id]
            if _on_manhattan_path(box_pos, goal_pos, bomb_pos):
                # The bomb is on the path — it needs to be moved (cost >= 1)
                # but we don't add penalty because the solver will pay for it.
                pass
...
def _candidate_stance_poses(
    ...
    for row, col in movables:
        for row_delta, col_delta, direction in DIRECTIONS:
            stance = (row - row_delta, col - col_delta)
            stance_row, stance_col = stance
            if (
                0 <= stance_row < rows
                and 0 <= stance_col < cols
                and stance not in walls
                and stance not in blocked
            ):
                poses.append((stance_row * cols + stance_col) * 4 + HEADING_INDEX[direction])
    return tuple(poses)
...
def solve_board(
    ...
    *,
    ...
    deadlock_cells = compute_deadlock_cells(board)
        ...
        for box_index, (pushed_box_id, box_pos) in enumerate(boxes_config):
            for row_delta, col_delta, direction in DIRECTIONS:
                ...
                if not (vanish and is_delivery):
                    ...
                    if use_deadlock and is_frozen_deadlock_positions(
...
def _is_deadlocked_dynamic(
    ...
    down = row + 1 >= board.rows or (row + 1, col) in walls
    left = col - 1 < 0 or (row, col - 1) in walls
    right = col + 1 >= board.cols or (row, col + 1) in walls
            ...
            for check_row in (top, top + 1):
                for check_col in (left, left + 1):
                    ...
                    is_wall = (
                        check_row < 0
                        or check_col < 0
                        or check_row >= board.rows
                        or check_col >= board.cols
                        or cell in walls
                    )
...
def _is_remaining_goal(board: Board, boxes_config: BoxConfig) -> bool:
    ...
    - If boxes_vanish_on_goal: true when no boxes remain (all delivered).
    - Else (legacy): true only when the (fixed) set of boxes are all exactly on their goals.
    """
    if board.level.boxes_vanish_on_goal:
        return len(boxes_config) == 0
    # legacy: all boxes (config should be full set) must be on their matching goals
    if len(boxes_config) != len(board.box_ids):
        return False
    for box_id, pos in boxes_config:
        if pos != board.goals.get(box_id):
            return False
    return True

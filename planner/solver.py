...
from .deadlock import compute_deadlock_cells, is_deadlocked_positions, is_frozen_deadlock_positions
from .grid import (
    Board,
    DIRECTIONS,
    Heading,
...
def _heuristic_with_distances(
    ...
    for box_id, box_pos in boxes_config:
        ...
        simple_sum += gd.get(box_pos, abs(box_pos[0] - goal_pos[0]) + abs(box_pos[1] - goal_pos[1]))
        if n > 1:
            row_costs: list[int] = []
            for gid in goal_ids:
                gpos = goals[gid]

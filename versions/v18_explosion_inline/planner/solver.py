    ...
    for goal_id, goal_pos in board.goals.items():
        ...
        while queue:
            ...
            for row_delta, col_delta, _direction in DIRECTIONS:
                pr, pc = cr - row_delta, cc - col_delta
                previous = (pr, pc)
                sr, sc = pr - row_delta, pc - col_delta
                stance = (sr, sc)
                if (
                    previous in dist
                    or pr < 0 or pc < 0 or pr >= rows or pc >= cols
                    or sr < 0 or sc < 0 or sr >= rows or sc >= cols
                    or previous in effective_walls
                    or stance in effective_walls
                ):
                    continue
                dist[previous] = cur_d + 1
                queue_append(previous)
        result[goal_id] = dist
...
def _heuristic_with_distances(
    ...
    """Tightest admissible heuristic: max of simple-sum and min-assignment.

    The simple-sum is the matched box->goal distance.  The min-assignment is
    the optimal box-to-goal matching.  Taking the max preserves admissibility
    while being tighter than either alone.
    """
    n = len(boxes_config)
    if n == 0:
        return 0
    ...
    return max(simple_sum, _min_assignment_cost(cost_matrix))
...
def _stance_lower_bound(
    ...
    Uses the tighter of: (1) L1 distance to nearest stance cell, (2) half-L1
    considering turn cost. This accounts for the fact that the car must face
    the right direction to push, which costs extra turns.
    """
    player_row, player_col = player
    best = 10**9
    for row, col in box_positions:
        # Cost to reach stance at (row-1, col) facing DOWN or (row+1, col) facing UP
        # = L1 + 1 turn (usually). Lower bound: L1.
        v1 = abs(player_row - (row - 1)) + abs(player_col - col)
        v2 = abs(player_row - (row + 1)) + abs(player_col - col)
        h1 = abs(player_row - row) + abs(player_col - (col - 1))
        h2 = abs(player_row - row) + abs(player_col - (col + 1))
        distance = min(v1, v2, h1, h2)
        if distance < best:
            best = distance
...
def _explosion_cells(board: Board, center: Position, walls: set[Position]) -> set[Position]:
    ...
    for r in range(row - 1, row + 2):
        for c in range(col - 1, col + 2):
            if 0 < r < rows - 1 and 0 < c < cols - 1 and (r, c) in walls:
                destroyed.add((r, c))
    return destroyed
            ...
            for row_delta, col_delta, direction in DIRECTIONS:
                ...
                if not (vanish and is_delivery):
                    ...
                    if use_deadlock and is_deadlocked_positions(
                        ...
                        pruned_deadlocks += 1
                        continue
                    # Frozen deadlock: if a group of boxes blocks each other (only 2+ boxes)
                    if use_deadlock and len(boxes_config) > 1 and is_frozen_deadlock_positions(
                        board, next_box_positions, goal_positions
                    ):
                        pruned_deadlocks += 1
                        continue

                # Corridor macro push: slide box through 1-wide tunnel
                other_box_positions = frozenset(p for _, p in boxes_config if p != box_pos)
                goal_for_box = board.goals[pushed_box_id]
                corridor_len = 0
                final_box = next_box
                if not (vanish and is_delivery):
                    corridor_len = _corridor_push_length(
                        board, next_box, row_delta, col_delta,
                        other_box_positions, board.walls,
                    )
                    if corridor_len > 0:
                        final_box = (next_box[0] + row_delta * corridor_len, next_box[1] + col_delta * corridor_len)
                        is_delivery = final_box == goal_for_box
                ...
                if vanish and is_delivery:
                    ...
                    temp[box_index] = (pushed_box_id, final_box)
                    next_config = tuple(temp)

                # Player ends up at the stance cell behind the box
                next_player = (final_box[0] - row_delta, final_box[1] - col_delta)
                next_state: State = (next_player, push_heading, next_config, hp)
                next_cost = cost_so_far + reach_costs[stance_pose] + 1 + corridor_len
                ...
                if box_h > 0:
                    ...
                    stance_lb = _stance_lower_bound(board, box_pos, remaining_box_pos_for_lb, ())
                    goal_prox = sum(
                        abs(p[0] - board.goals[bid][0]) + abs(p[1] - board.goals[bid][1])
                        for bid, p in next_config
                    ) * 0.01
                    priority = next_cost + box_h + stance_lb + goal_prox
                else:
                    priority = next_cost
...
def solve_bombs(board: Board, *, max_expanded: int = DEFAULT_MAX_EXPANDED) -> SolveResult:
    ...
    while frontier and expanded < adaptive_limit:
        ...
        walls_frozen = frozenset(walls_tuple)
        ...
        for box_index, (pushed_box_id, box_pos) in enumerate(boxes_config):
            for row_delta, col_delta, direction in DIRECTIONS:
                ...
                # Corridor macro push for bomb solver
                other_box_positions = frozenset(p for _, p in boxes_config if p != box_pos)
                corridor_len = 0
                final_box = next_box
                if not (vanish and is_delivery):
                    corridor_len = _corridor_push_length(
                        board, next_box, row_delta, col_delta,
                        other_box_positions, walls_frozen,
                    )
                    if corridor_len > 0:
                        final_box = (next_box[0] + row_delta * corridor_len, next_box[1] + col_delta * corridor_len)
                        is_delivery = final_box == board.goals[pushed_box_id]
                ...
                if box_h > 0:
                    remaining_box_pos_for_lb = [p for _, p in next_config]
                    stance_lb = _stance_lower_bound(board, box_pos, remaining_box_pos_for_lb, bombs_tuple)
                    goal_prox = sum(
                        abs(p[0] - board.goals[bid][0]) + abs(p[1] - board.goals[bid][1])
                        for bid, p in next_config
                    ) * 0.01
        ...
        for bomb_pos in bombs_tuple:
            for row_delta, col_delta, direction in DIRECTIONS:
                ...
                if box_h > 0:
                    ...
                    stance_lb = _stance_lower_bound(board, bomb_pos, remaining_box_pos_for_lb, next_bombs_tuple)
                    priority = next_cost + box_h + stance_lb
                else:
                    priority = next_cost
                heappush(frontier, (priority, next_cost, next(serial), next_state))
                generated += 1
...
def _is_deadlocked_dynamic(
    ...
    # Wall-line deadlock with box obstacles: if box is against a wall line and
    # there's a box blocking along that line toward the goal, it's deadlocked
    if up and not down:
        # Box against top wall - can only move left/right
        # Check if a box blocks the path toward goal along the row
        dc = 1 if goal[1] > col else -1
        nc = col + dc
        while 0 <= nc < cols:
            if (row, nc) in walls:
                break
            if (row, nc) in box_positions and (row, nc) not in goal_positions:
                return True  # box blocking path toward goal
            nc += dc
    ...
    if left and not right:
        dr = 1 if goal[0] > row else -1
        nr = row + dr
        while 0 <= nr < rows:
            if (nr, col) in walls:
                break
            if (nr, col) in box_positions and (nr, col) not in goal_positions:
                return True
            nr += dr
...
def _heuristic_dynamic(
    ...
    While bombs exist, walls may change, so use Manhattan (safe lower bound).
    Once bombs are gone, use tighter wall-aware BFS distances.
    """
    if not bombs_tuple:
        return _heuristic_with_distances(
            board,

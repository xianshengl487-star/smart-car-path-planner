...
def is_deadlocked_positions(
    ...
    # Corner deadlock: box in a corner (both sides of one corner blocked)
...
def _is_corner(board: Board, pos: Position) -> bool:
    ...
    left = col - 1 < 0 or (row, col - 1) in walls
    right = col + 1 >= cols or (row, col + 1) in walls
    # Corner: two perpendicular walls meeting at this cell
...
def is_frozen_deadlock_positions(
    ...
    # A box on any goal is never considered frozen
    candidates = box_positions - goal_positions
    if not candidates:
        return False

    # Quick check: if any candidate has a completely free direction, it's not frozen
    # (avoid expensive fixed-point iteration)
    ...
    while changed and iteration < max_iter:
        ...
        for pos in candidates:
            ...
            for dr, dc in deltas:
                ...
                # If another box blocks target or stance, treat as blocked only if that box is already frozen
                ...
                if target_blocked or stance_blocked:
                    continue
                # Both target and stance are free → direction NOT blocked
